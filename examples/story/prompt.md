...
voice as literal glue. If the same closed choice must stay aligned across beats, reuse the identical
`{a|b|c}` text and set the same `syncGroup`. Identical blocks are not autosynced. Do not parametrize every
